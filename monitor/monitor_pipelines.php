...
/**
 * Ajout des scripts de dc-js au texte (un head) transmis
 *
 * @param  string $flux  Contenu du head
 ...
 */
function monitor_inserer_head($flux, $prive = false){
	$js_start = find_in_path('lib/dc.js/web/js/dc.js');
	$js_start2 = find_in_path('lib/dc.js/web/js/crossfilter.js');
	if (defined('_VAR_MODE') AND _VAR_MODE=="recalcul") {
		$js_start = parametre_url($js_start, 'var_mode', 'recalcul');
		$js_start2 = parametre_url($js_start, 'var_mode', 'recalcul');
	}
	$flux .= 
		  "<script type='text/javascript' src='$js_start'></script>\n";
	$flux .= 
		  "<script type='text/javascript' src='$js_start2'></script>\n";
